...
#  Version : 1.0
#  Action du script : Permet de nettoyer votre navigateur
...
if ($list) {
    ...
    ""
    "-------------------"
    # Clear Terminal Server Client Cache
    Write-Host -ForegroundColor Green "SECTION 6: Terminal Server Client Cache"
     "-------------------"
    Write-Host -ForegroundColor yellow "Clearing Terminal Server Client Cache"
    Write-Host -ForegroundColor cyan
    Import-CSV -Path C:\users\$env:USERNAME\users.csv | foreach {
